...
static void update_convergency_file(u32 last_round) {

   u32 seed_num;
   seed_num = current_entry;

   fprintf(convergency_file,
	   "%u %u\n", seed_num, last_round);

   fflush(convergency_file);
...
static void show_stats(void) {
  ...
  if (fuzzing_mode == 1) {
	  if (local_best_greywolf_mode) {
		  ...
		  sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN
			  " (%s)", crash_mode ? cPIN "peruvian were-rabbit" :
			  cYEL "FA-AFL + firefly disturb", use_banner);
	  }
...
static u8 deterministic_fuzz_one(char** argv) {
...
static u8 normal_fuzz_one(char** argv) {
...
EXP_ST void setup_dirs_fds(void) {
  ...
  /* Generate convergency data

  tmp = alloc_printf("%s/convergency_data", out_dir);
  fd = open(tmp, O_WRONLY | O_CREAT | O_EXCL, 0600);
  if (fd < 0) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  %convergency_file = fdopen(fd, "w");
  %if (!convergency_file) PFATAL("fdopen() failed");

  fprintf(convergency_file, "# seed_num, round_last_moved\n");
 

  prob_file = fdopen(fd, "w");
  if (!prob_file) PFATAL("fdopen() failed");*/
